...
class KartuStockController extends Controller
{
    ...
    public function search(Request $request){
        ...
        if($request->division_id == ''){
            ...
            $data['repair'] = RepairStock::where('colorcard_id', $id->id)->whereNull('division_id')->whereBetween('date', [$request->dari, $request->sampai])->get();
            $data['lokasi'] = 'Pusat';
        } else {
            //$data['receive'] = Receive::where('colorcard_id', $id->id)->whereBetween('date', [$request->dari, $request->sampai])->get();
            ...
            $data['repair'] = RepairStock::where('colorcard_id', $id->id)->where('division_id', $request->division_id)->whereBetween('date', [$request->dari, $request->sampai])->get();
